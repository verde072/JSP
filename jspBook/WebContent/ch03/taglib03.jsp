...
JSTL : JSP Standard Tag Library
 ...
 -->
	...
	<!-- 
		gt : greater than 크다
		lt : less than 작다
		ge : greater than or equals 이상
		le : less than or equal 이하
	 -->
	<c:if test="${num3 > 10}">10 초과입니다.</c:if>
	<c:if test="${num3 gt 10}">10 초과입니다.</c:if>
	<c:if test="${num3 <= 10}">10 이하입니다.</c:if>
	<c:if test="${num3 le 10}">10 이하입니다.</c:if>
	<hr />
	<c:set var="score" value="9" />
	<c:choose>
		<c:when test="${score==0}"> 0점</c:when>
