...
<% //스크립틀릿
	...
	//해당 패키지의 DiskFileUpload 클래스를 통해 객체를 생성해야 함
	DiskFileUpload upload = new DiskFileUpload(); 
	//fileupload01.jsp의 form 안에 들어있는 폼데이터(multipart/form-data유형)를 가져옴
	...
	while(params.hasNext()){
		...
		//1) 일반데이터(title) 
		// isFormField()메소드 : 요청 파라미터가 파일이 아니라 일반 데이터인 경우 true
		if(item.isFormField()){
			//title:개똥이
			String name = item.getFieldName();//title
			String title = item.getString("UTF-8");//개똥이
		...
		//2) 파일객체(filename)
			...
			//클라이언트(크롬에서 불러온 사용자의 컴퓨터 안에 있던 이미지) -> 서버(톰켓서버의 c드라이브의 upload폴더에 이미지)
			//계획에따라 복사 실행
			item.write(file);
		}
	}
